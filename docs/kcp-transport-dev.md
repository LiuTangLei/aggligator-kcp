...
验证 KCP 作为唯一 transport 独立工作:

```bash
# 服务端仅使用 KCP
cargo run --bin agg-tunnel -- server --kcp 5801 -p 22

# 客户端仅使用 KCP
cargo run --bin agg-tunnel -- client --kcp 127.0.0.1:5801 -p 22:2222
```

### 9.3 TCP + KCP 混合聚合测试

验证 KCP 和 TCP 可以同时作为 transport 聚合使用:

```bash
# 服务端同时监听 TCP 和 KCP
cargo run --bin agg-tunnel -- server --tcp 5800 --kcp 5801 -p 22

# 客户端同时使用 TCP 和 KCP 链路 (聚合)
cargo run --bin agg-tunnel -- client --tcp 127.0.0.1:5800 --kcp 127.0.0.1:5801 -p 22:2222
```

### 9.4 TCP + KCP 混合聚合: 技术分析

Aggligator 的核心设计就是将多种异构传输的链路聚合为一个逻辑连接。
TCP + KCP 混合聚合在架构上完全支持，两者也都可以单独使用。
...
客户端                              服务端
┌──────────────┐                ┌──────────────┐
│  Aggligator  │── TCP link ──→│  Aggligator  │
│  聚合层      │── KCP link ──→│  聚合层      │
│  (按序号分发) │                │  (按序号重组) │
└──────────────┘                └──────────────┘
```

- 发送端：Aggligator 将数据分包编序号，分发到所有可用链路（TCP/KCP）
- 接收端：按序号重组还原数据，不关心包来自哪种传输协议
- 每条链路独立提供可靠字节流（TCP 由 OS 保证，KCP 由自身 ARQ 保证）

#### 不会导致额外丢包

TCP 和 KCP 各自在传输层保证可靠投递，交给 Aggligator 的数据是完整的。
底层网络丢包由各自重传机制处理，不会"传染"到其他链路。

#### 需注意的问题

| 问题 | 说明 | 缓解方式 |
|------|------|----------|
| **延迟不对称队头阻塞** | KCP(fast_mode) 延迟低于 TCP。收端按序号重组时，快链路(KCP)的包可能等待慢链路(TCP)的包 | Aggligator `Cfg` 中有 reorder timeout 配置，超时后跳过等待 |
| **吞吐量分配不均** | TCP 拥塞控制较保守，KCP 更激进，流量可能偏向 KCP | Aggligator 内部按链路容量自适应分配；极端场景下可能 TCP 利用不足 |
| **拥塞控制交互** | KCP 的激进重传可能占用带宽，间接影响共享同一物理链路的 TCP 流 | 如果 TCP 和 KCP 走同一物理网络，需关注。走不同网络时无影响 |
| **故障隔离** | 一条链路故障不影响其他链路，这是聚合的核心优势 | 链路断开后 Aggligator 自动在剩余链路上继续工作 |

#### 最佳实践

- **同一网络下**: TCP+KCP 聚合的主要价值是冗余容错，而非提速（共享带宽）
- **不同网络下**: 如 TCP 走有线 + KCP 走 4G，可同时获得提速和容错
- **测试建议**: 先分别单独测试 TCP 和 KCP 可用性，再测试混合聚合
